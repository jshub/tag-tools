...
#   end
  ...
  def initialize(path=nil)
    ...
    if !path
      # load the browsers config
      data = YamlConfig.get :browsers
      # check we have the data for this browser
      if !data['firefox']
        puts "WARNING: No path to #{self} setup in config/javascript_test_autotest.yml"
        exit
      end
  ...
  def visit(url)  
    system("open -a #{@path} '#{url}'") if macos?
    ...
    task @name do
      ...
      @browsers.each do |browser|
        if browser.supported?
          ...
          failures = []
          browser.setup
          puts "\nStarted :#{@name} task with #{@tests.size} tests in #{browser}"
          @tests.each do |test|
...
# Load configuration data from the javascript_test_autotest config file
require 'yaml'
class YamlConfig
  ...
  private
    def self.configs
      unless defined? @@configs
        ...
        # check we have loaded some data
        if @@configs.empty?
          puts "WARNING: No config setup in config/javascript_test_autotest.yml"
          exit
        end
      end
      @@configs
